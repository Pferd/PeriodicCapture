...
// call back function to the drawing area.
static gboolean
da_expose(GtkWidget *drawArea, GdkEvent *event, gpointer data)
{    
 //nothing
    
} // end of Drawing area call back function.
...
void show1(GtkWidget *widget, gpointer drawArea){

 //char *buf="Capture Started!\n";
 //gtk_label_set_text(GTK_LABEL(label),buf);
    ...
    printf("we are inside the Da AREAS");
    /* Create pixbuf */
...
void capture1(GtkWidget *widget, gpointer drawArea){
...
//printf("Im after g signal DA\n");
//update the drawing area! -- TO-DO -- here
...
// Apparently, widgets are hooked to the frame.
  ...
  g_signal_connect(capture, "clicked", G_CALLBACK(capture1),drawArea);

  g_signal_connect(show, "clicked", 
  G_CALLBACK(show1), drawArea); 
